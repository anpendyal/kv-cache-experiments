"""
Benchmarks KV cache size before and after KVzipPress compression
...
Runs two forward passes per case (prefill only, no generation):
  ...
  2. KVzipPress — compressed KV cache size
"""
...
def setup_logger(log_path: str) -> logging.Logger:
    logger = logging.getLogger("kv_press_bench_zip")
...
def run(reporters, logs_dir, max_tokens, compression_ratio, limit):
    ...
    log_path = os.path.join(job_dir, f"{job_id}_zip_bench.log")
    sys.stderr = open(os.path.join(job_dir, f"{job_id}_zip_bench_error.txt"), "a", encoding="utf-8")
    logger = setup_logger(log_path)

    logger.info(f"press=KVzipPress | compression_ratio={compression_ratio} | max_tokens={max_tokens}")

    logger.info(f"Loading model {MODEL_ID!r} with attn_implementation=sdpa...")
    tokenizer = AutoTokenizer.from_pretrained(MODEL_ID)
    model = AutoModelForCausalLM.from_pretrained(MODEL_ID, dtype=torch.bfloat16, device_map="auto", attn_implementation="sdpa")
    model.eval()
